    ...
    async fn grant_share_object(
        ...
        loop {
            ...
            // Grant the object privilege by transaction.
            {
                ...
                // condition
                condition.push(txn_cond_seq(share_name_key, Eq, share_id_seq));
                ...
                // if_then
                if_then.push(txn_op_put(&id_key, serialize_struct(&share_meta)?)); /* (share_id) -> share_meta */
                if_then.push(txn_op_put(&grant_object, serialize_struct(&share_ids)?)); /* (object) -> share_ids */

                // grant reference tables privileges
                for (table_name, db_id, reference_table) in &reference_table_info {
                    let ref_object = ShareObject::Table(
                        table_name.to_string(),
                        *db_id,
                        reference_table.ident.table_id,
                    );
                    ...
                    let ref_grant_object = ShareGrantObject::new(&ref_object);
                    let res = get_object_shared_by_share_ids(self, &ref_object).await?;
                    let ref_share_ids_seq = res.0;
                    let mut share_ids: ObjectSharedByShareIds = res.1;
                    share_ids.add(share_id);

                    condition.push(txn_cond_seq(&ref_grant_object, Eq, ref_share_ids_seq));
                        ...
                        reference_tables: if reference_table_info.is_empty() {
                            ...
                            Some(
                                reference_table_info
                                    .into_iter()
                                    .map(|(_, db_id, table_info)| (db_id, table_info))
                                    .collect(),
                            )
                        },
                    });
...
pub async fn check_access_reference_tables(
...
) -> Result<Vec<(String, u64, TableInfo)>, KVAppError> {
    ...
    for (db, table) in reference_tables {
        ...
        for ref_db in &share_meta.reference_database {
            ...
            for ref_table in &share_meta.reference_table {
    ...
    for (table_name, db, ref_table) in ref_tables {
        ...
        if let Some(table_meta) = table_meta {
            ...
            reference_table_info.push((table_name, ref_table.db_id, table_info));
        } else {
            return Err(KVAppError::AppError(AppError::UnknownTable(
                UnknownTable::new(
                    ref_table.name.clone(),
                    format!("check_access_reference_tables: {}", ref_table.name),
                ),
            )));
        }
    }
